...
public interface PreScoreMapper {
    ...
    @Insert("insert into  pre_score (uid, session,`group`,zone, seat_num, sign_num, written_score, practical_score, q_and_a_score, final_score, ranking) " +
            "value (#{uid},#{session},#{group},#{zone},#{seatNum},#{signNum},#{writtenScore},#{practicalScore},#{qAndAScore},#{finalScore},#{ranking})")
